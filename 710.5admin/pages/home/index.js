...
Page({
      ...
      success: (res) => {
        ...
        wx.getStorage({
          ...
          success(res) {
            ...
            let id = res.data
            wx.request({
              url: 'http://81.68.194.42:9090/api/1/list/page/2',
              data: {
                "size": 10, //分页参数
                "page": 1, //分页参数
            ...
            //获取待审核数量
            wx.request({
              url: 'http://81.68.194.42:9090/api/1/check/num/1',
              header: {
                'content-type': 'application/json',
                'token': token
              },
  ...
  switchNav(event) {
    ...
    if (this.data.currentTab == cur) {
      ...
      let num = cur + 1
      wx.request({
        url: 'http://81.68.194.42:9090/api/1/check/num/' + num,
        header: {
          'content-type': 'application/json',
          'token': that.data.token
        },
  ...
  grouplistchangeconfirm(e) {
    ...
    for (let i = 0; i < checklist.length; i++) {
      ...
      if (i == checklist.length - 1) {
        wx.request({
          url: 'http://81.68.194.42:9090/api/1/change/group/2',
          data: {
            "ids": caridarr,
  ...
  deleteitem() {
    ...
    for (let i = 0; i < checklist.length; i++) {
      ...
      if (i == checklist.length - 1) {
        wx.request({
          url: 'http://81.68.194.42:9090/api/1/remove/task/batch',
          data: {
            "ids": caridarr,
          },
          header: {
